 ...
 * partner email + phone, and a smooth WhatsApp hover interaction.
 */
export function ContactPartnerCard({ className = "" }: ContactPartnerCardProps) {
  ...
  return (
    ...
    >
      <div className="group relative isolate w-full max-w-2xl overflow-hidden rounded-3xl border border-surface-white/15 shadow-[0_16px_40px_-22px_rgba(61,26,92,0.45)] transition-[transform,box-shadow] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] will-change-transform hover:-translate-y-1 hover:shadow-[0_24px_48px_-20px_rgba(61,26,92,0.58)]">
        {/* Footer background */}
        ...
        <div className="relative flex flex-col sm:flex-row items-center gap-5 sm:gap-6 px-6 sm:px-8 py-6 sm:py-7">
          {/* Logo mark */}
          <div className="relative shrink-0 flex items-center justify-center w-[4.5rem] h-[4.5rem] sm:w-20 sm:h-20 rounded-2xl bg-surface-white/10 border border-surface-white/20 backdrop-blur-md shadow-inner transition-[transform,border-color] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.03] group-hover:border-secondary-fixed/40">
            <Image
          ...
          {/* Copy */}
          <div className="min-w-0 flex-1 text-center sm:text-start text-surface-white">
            ...
            <p className="text-xl sm:text-2xl font-semibold leading-tight tracking-tight">
              {contactPartner.name}
            </p>
            <a
              href={`mailto:${contactPartner.email}`}
              className="mt-1.5 inline-flex items-center gap-1.5 text-xs sm:text-sm text-surface-white/70 hover:text-secondary-fixed transition-colors"
              dir="ltr"
            >
              {/* <MaterialIcon name="mail" className="text-sm" /> */}
              {contactPartner.email}
            </a>
            <p className="mt-1 block text-xs sm:text-sm text-surface-white/70" dir="ltr">
          ...
          <a
            ...
            className="shrink-0 inline-flex items-center gap-2.5 rounded-full bg-[#25D366] text-[#053b1c] pl-3.5 pr-4 py-2.5 shadow-[0_8px_24px_-6px_rgba(37,211,102,0.55)] transition-[transform,background-color,box-shadow] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] hover:bg-[#2fe374] hover:shadow-[0_10px_26px_-6px_rgba(37,211,102,0.75)] hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary-fixed focus-visible:ring-offset-2"
          >
